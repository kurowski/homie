...
- **`homie help <cmd>`** has a fleshed-out `Long` description for every
...
belongs in `homie help` and the Hugo site instead.
...
- Go source for the `homie` binary
...
Generated by `homie init`. Contains the user's `homie.toml`, `home/` tree,
...
The `bootstrap.sh` generated by `homie init` does the following:

1. Detects OS and CPU architecture
2. Downloads the appropriate `homie` binary from
   `github.com/kurowski/homie/releases`
3. Clones the user's environment repo if not already present
4. Runs `homie apply` from the repo root

Every child that can prompt (`homie bootstrap`, `git clone`, `homie apply`) is
handed `/dev/tty` when one exists, because under `curl … | bash` the
inherited stdin is the pipe carrying the script — so `sudo` inside a setup
script would otherwise die with "a terminal is required to read the
password" and, under `set -euo pipefail`, take the rest of the run with it
(#46). It has to be a per-command redirect, not a script-wide `exec
</dev/tty`: bash is still reading the remaining script text from that same
stdin. The `/dev/tty` probe sits in a subshell because a failed redirection
...
**tool-owned** file (`entry.toolOwned`): it encodes how *this* `homie`
...
`homie init --update` (`cmd/homie/init_update.go`, `internal/scaffold/update.go`)
...
```
# homie:generated version=v0.5.2 sha256=<digest of the file, stamp line excluded>
```
...
**No state.** Every `homie apply` is a full reconciliation pass. Steps are
...
homie/
  cmd/
    homie/          # cobra commands; one file per subcommand
  internal/
    ...
    scaffold/       # `homie init` — generate a new user environment repo
    selfupdate/     # `homie selfupdate` — replace the binary with the latest release
    repo/           # locate the user repo (HOMIE_REPO / walk-up)
...
whole system, which isn't a decision `homie apply` gets to make. So Install
...
`homie apply`/`homie install`, and `homie doctor`'s `checkPackages`, warn and skip
...
- `tree.Classify(repoDir, base, tags)` — same walk, also returns
  inactive trees (used by `homie doctor`).
...
When you change user-visible behavior, update both `homie help <cmd>` and
...
- **v0.2.0** — the `homie home` command, charmbracelet/fang-styled help and
...
- **v0.4.0** — authoring and agent introspection, plus the externals phase:
  render previews (`homie render <path>`, `homie home --dry-run`); machine-readable
  state (`homie status --json`, `homie doctor --json`, and `homie context` — the exact
  template data model as JSON, keys matching the template fields verbatim);
  an offline `homie help templating` reference (aliases `template`/`templates`);
...
- **v0.4.1** — `homie selfupdate` (alias `self-update`): replace the running
...
- **v0.5.0** — Termux (Android) as a first-class platform (`TERMUX_VERSION` →
  ...
  hardened.
- **v0.5.1** — a dogfooding fix to the scaffolded `bootstrap.sh`: hand the
  controlling terminal to every child that can prompt, so `sudo` works under
  ...
  in the FAQ.
- **v0.5.2** — `homie init --update`: the repeatable path for getting a fix like
...
- **v0.5.5** — reporting and test-hygiene follow-ups: `current` names the homie
