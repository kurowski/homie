...
- `home.tag-X[.tag-Y...]/` — tag-conditional siblings of `home/`
- `hosts/<name>.toml` — optional per-host overlay merged onto `homie.toml`
- `scripts/` — ordered setup steps (`pre-*.sh` run before packages)
...
```
dotfiles/             # (repo name is user's choice; "dotfiles" is conventional)
  bootstrap.sh
  homie.toml
  ...
  home.tag-work/      # only when hasTag "work"
    .config/...
  hosts/              # optional per-host overlays
    coach.toml
  scripts/
    pre-01-repos.sh   # runs before [packages]
    01-shell.sh
    02-tools.sh
  README.md
```
...
Templates live in `home/` (alongside plain symlinkable files) and use the
`.tmpl` extension plus Go's `text/template` syntax augmented with
[Sprig](https://masterminds.github.io/sprig/) helpers.
`home/.gitconfig.tmpl` renders to `~/.gitconfig` with the suffix stripped.
...
A user repo has a single `home/` directory that mirrors `$HOME`. Inside:
plain files are symlinked into `$HOME` at the matching path; files ending
in `.tmpl` are rendered through Go `text/template` + Sprig, with the
`.tmpl` suffix stripped from the output path. The suffix is the only
disambiguator — no separate `dotfiles/` and `templates/` directories.

Sibling directories named `home.tag-<X>[.tag-<Y>...]` are processed only
when every named tag is active. Plain `home/` always applies.
...
home.tag-work/                  # only when hasTag "work"
  ...
  .ssh/config.tmpl
home.tag-work.tag-kde/          # AND: only when both tags active
...
render, packages (apt + dnf + flatpak + brew), runner, UI, `hm apply`
end-to-end, `hm init` scaffold, `bootstrap.sh` template, `hm status` /
`hm doctor`, GitHub Actions release pipeline, e2e container harness
covering Ubuntu/Debian/Fedora, and the docs site at homie.sh.

Post-v0.0.2 features merged but not yet tagged: multi-host overlay
(`hosts/<name>.toml` + hostname auto-tag), pre-package scripts
(`scripts/pre-*.sh`), tag-keyed packages (`[packages."tag:X"]`),
tag-conditional trees (`home.tag-X/`), pluggable backends (flatpak,
brew), and the unified `home/` tree replacing the older `dotfiles/` +
`templates/` split.

**Layout migration** (one-time, for repos created before the unified
tree landed): `git mv dotfiles/* home/ && git mv templates/* home/ &&
rmdir dotfiles templates`. The `.tmpl` suffix is what now distinguishes
"render this" from "symlink this," so the templates keep their suffix
and everything else stays plain.
