...
their children. To fix a repo scaffolded before that, either re-run `hm
init` in a scratch directory and copy the new `bootstrap.sh` over, or
change the last line by hand:

```sh
cd "$REPO_DIR"
if (: </dev/tty) 2>/dev/null; then
  exec hm apply </dev/tty
fi
exec hm apply
```

The probe belongs in a subshell — `exec` is a special builtin, so a
failed redirection on it can kill a non-interactive shell outright. CI
and containers have no `/dev/tty`, fall through to the plain `exec`, and
are unaffected.
